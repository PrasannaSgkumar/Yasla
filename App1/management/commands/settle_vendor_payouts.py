...
#             try:
#                 # Ensure the fund_account_id is properly included in the request URL
#                 resp = requests.post(
#                     API_BASE.format(bank.razorpay_fund_account_id),  # Ensure this is correctly formatted
#                     json=payload,
...
from App1.models import Salon, BankDetails, Vendor_Payment  # Adjust import as needed

log = logging.getLogger(__name__)

# Razorpay API Base URL for Payouts
API_BASE = "https://api.razorpay.com/v1/payouts"  # This should be the correct endpoint

def _previous_week_range(today: date):
    """Get the previous week's Monday to Sunday range."""
    monday_this_week = today - timedelta(days=today.weekday())  # Find the Monday of the current week
    week_start = monday_this_week - timedelta(days=7)  # Previous week start
    week_end = week_start + timedelta(days=6)  # Previous week end
    return week_start, week_end

def _week_number(week_start: date):
    """Generate a unique week number."""
    return int(week_start.strftime("%Y%W"))
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--force",
            action="store_true",
            help="Run regardless of the weekday (for manual backfills/tests).",
        )
        parser.add_argument(
            "--date",
            type=str,
    ...
    def handle(self, *args, **opts):
        ...
        # Pull unpaid vendor payments for the previous week
        payments = (
            Vendor_Payment.objects
            .select_related("salon")
            .filter(
        ...
        for salon_id, bundle in by_salon.items():
            ...
            amount_paise = _paise(total_amount)  # Convert total amount to paise
            reference_id = f"salon-{salon_id}-wk-{week_no}"
            idempotency_key = f"{salon_id}-{week_start.isoformat()}-{week_end.isoformat()}"

            # Prepare payload for payout
            # Razorpay API Base URL for Payouts (correct format with fund account ID)
        API_BASE = "https://api.razorpay.com/v1/payouts"  # Correct endpoint
        ...
        # Debug: Print the Fund Account ID to ensure it's correct
        print(f"Fund Account ID: {bank.razorpay_fund_account_id}")

        # Construct the full URL for the payout request
        api_url = f"https://api.razorpay.com/v1/fund_accounts/{bank.razorpay_fund_account_id}/payouts"
        print(f"Making payout request to: {api_url}")
        ...
        try:
            ...
            # If the payout was successful, update the database
            if resp.status_code in (200, 201):
                data = resp.json()
                payout_id = data.get("id")
                payout_status = data.get("status")  # queued, processing, processed, reversed, etc.
                with transaction.atomic():
                    for row in bundle["rows"]:
                        row.transaction_id = payout_id
                        row.payout_done = True  # Update payout status
                        row.save(update_fields=["transaction_id", "payout_done", "updated_at"])
                self.stdout.write(self.style.SUCCESS(
                    f"Payout created id={payout_id} status={payout_status} for salon={salon.salon_name}"
                ))
                ...
                failures += 1
                self.stderr.write(self.style.ERROR(
                    f"Payout failed for salon={salon.salon_name}: {resp.status_code} {resp.text}"
                ))
